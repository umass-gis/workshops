...
---
## If I learn QGIS, will I be able to use other GIS programs, like ArcGIS?
{:toc}

QGIS and ArcGIS are the most common desktop GIS applications out there at the moment. They do have some differences that can make switching between them a small challenge:
...
A self-paced course that I’d recommend is the [GIS Practicum: Introduction to GIS Using Open Source Software](https://www.baruch.cuny.edu/confluence/display/geoportal/GIS+Practicum), put together by Frank Donnelly at Baruch College CUNY. This is a full-day workshop that you can work through on your own.
...
* map shapes, since vectors are scalable and adjust visually when you zoom in/out,
* store more than one kind of information about each shape.<br>

While rasters are better if you want to:
* map a single kind of information over large areas (e.g. elevation, rainfall, land use).

## Can I add a new column to the attribute table?
{:toc}

There are two ways to add a new column to an attribute table. First, open the attribute table and toggle on editing mode (by clicking the pencil button). Then:
1. Click the `New field` button (table with a yellow column) and fill out the `Add field` dialog. Make sure the `Type` is set to the data type you want - you won’t be able to change this. Click `OK`. The new field is added to the right side of the attribute table. As long as you’re still in editing mode, you can manually enter information into any of the new squares. Or you can use the field calculator (abacus button) to quickly calculate values for all the records at the same time - just make sure `Update existing field` is checked and that you’ve selected the name of the field you just created.
...
You can practice adding a new field with the latter method in the [Visualize the census data](https://umass-gis.github.io/workshops/content/basics-qgis/tutorial.html#visualize-the-census-data) part of the tutorial!
...
## Does the Project CRS need to match the CRS of the data layers?
{:toc}

Does it need to? No. Are some of your layers not showing up, or does the map look wrong in some way? It could certainly help!<br>
...
## How can I keep track of the metadata for the files I create? Does QGIS have tools that allow me to manage files directly?
{:toc}

Metadata is essentially the explanatory text that accompanies a file, letting people know information about it like who created it and when, why, etc. Here are some easy ways to keep track of metadata: 
...
## How well optimized is QGIS (and plugins) for use in parallel analyses on High Performance Computing Clusters (HPCCs)?
{:toc}

Desktop QGIS software is really best for working with GIS data on a single computer, such as your personal laptop or a remote desktop that you’re accessing through a VPN. <br>

If you’re interested in working with GIS data at a bigger scale (i.e. the *big data* scale), you’ll likely want to use different tools that require a more advanced technical skillset to deploy. For example, a common open-source setup is [PostgreSQL](https://www.postgresql.org/) with [PostGIS](https://postgis.net/). These articles could be helpful resources if you’d like to dive into this topic further: [Geospatial Big Data Handling with High Performance Computing: Current Approaches and Future Directions (Li 2019)[https://arxiv.org/abs/1907.12182v1], [Parallel Processing Strategies for Big Geospatial Data (Werner 2019)](https://doi.org/10.3389/fdata.2019.00044).
